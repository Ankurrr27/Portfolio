...
const About = ({ totalViews = 0 }) => {
  ...
  const socials = [
    { icon: Mail, url: `mailto:${p.email || fallbackProfile.email}`, label: "Email", color: "text-amber-500" },
      ...
      <div className="section-container grid grid-cols-1 gap-12 lg:grid-cols-12 lg:items-center">
        ...
        <div className="order-2 space-y-8 lg:order-1 lg:col-span-7">
          <div className="space-y-6">
            ...
            <button
              ...
              className="inline-flex min-h-11 items-center justify-center rounded-lg border border-zinc-800 bg-zinc-900 px-4 text-sm font-bold text-zinc-300 transition-colors hover:border-amber-500/50 hover:text-white md:hidden"
            ...
            <div className="grid grid-cols-3 gap-2 pt-4 lg:grid-cols-6">
              {socials.map(({ icon: Icon, url, label, color }) => (
                <a
                  ...
                  className="group flex min-h-12 items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/[0.055] px-2 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-xl transition-all hover:border-amber-500/40 hover:bg-amber-500/10"
                >
        ...
        >
          <ProfileCard
            ...
            behindGlowEnabled
            behindGlowColor="rgba(245, 158, 11, 0.66)"
            behindGlowSize="52%"
            innerGradient="linear-gradient(145deg, rgba(245,158,11,0.36) 0%, rgba(251,191,36,0.22) 55%, rgba(9,9,11,0.9) 100%)"
            onContactClick={next}
          />
          ...
          <div className="mt-4 flex items-center justify-center gap-2">
            <button
              type="button"
              onClick={prev}
              className="flex min-h-10 min-w-10 items-center justify-center rounded-lg border border-white/10 bg-white/[0.06] text-zinc-400 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-xl transition-colors hover:border-amber-500/50 hover:text-amber-500"
              ...
              {images.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    i === carouselIndex ? "w-7 bg-amber-500" : "w-2 bg-zinc-600 hover:bg-zinc-400"
                  }`}
