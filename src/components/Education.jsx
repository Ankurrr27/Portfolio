...
const EducationItem = ({ institution, period, detail, degree, isVisible, index, imageUrl, skills, description }) => {
  ...
  return (
    <div
      className={`panel group relative w-full transition-all duration-500 overflow-hidden flex flex-col md:flex-row hover:border-indigo-500/50 hover:shadow-[0_20px_50px_rgba(99,102,241,0.15)] hover:-translate-y-1 ${
    ...
    >
      ...
      <div className="flex-1 p-6 md:p-10 flex flex-col justify-center relative z-20 bg-white/0 dark:bg-zinc-900/10 backdrop-blur-3xl">
         ...
         <div className="flex flex-wrap items-center gap-2 mb-5">
            <span className="chip text-indigo-400 border-indigo-500/30 bg-indigo-500/5 flex items-center gap-1.5 px-3">
         ...
         {/* Title */}
         <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white tracking-tight leading-tight mb-4 group-hover:text-indigo-400 transition-colors duration-300 break-words">
           {institution}
         </h3>
         ...
         {skillList.length > 0 && (
            <div className="flex flex-wrap gap-2.5 mt-auto">
               {skillList.map((skill, idx) => (
                 <span 
                   key={skill} 
                   className="chip bg-zinc-800/30 border-zinc-700/50 hover:border-indigo-500/50 hover:text-indigo-400 hover:bg-indigo-500/5 transition-all duration-300 cursor-default"
                   style={{ 
        ...
        <div className={`flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-12 md:mb-16 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
           <div className="space-y-4">
              <div className="section-kicker bg-indigo-500/10 border-indigo-500/20">
                 ...
                 <span className="text-indigo-300">Curriculum Vitae</span>
              </div>
