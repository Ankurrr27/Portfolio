      ...
      <div className="flex flex-col w-full relative">
          ...
          <div className="flex items-center gap-4 md:gap-6 flex-1 min-w-0 w-full">
            <div className="shrink-0 relative group/logo">
              {logoUrl ? (
                <div className="relative">
                  ...
                  <div className="absolute inset-0 bg-amber-500/10 opacity-0 group-hover/logo:opacity-100 transition-opacity rounded-xl" />
                </div>
              ) : (
                <div className="icon-box flex items-center justify-center w-12 h-12 md:w-16 md:h-16 rounded-xl bg-zinc-900 border border-zinc-800 text-amber-400">
                  <ShieldCheck size={24} />
                </div>
              )}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-amber-400 opacity-90">
                  {period}
                </span>
              </div>
            </div>
        ...
        <div className="p-6 md:p-10 relative z-10">
          <div className="relative space-y-8 md:space-y-12 pl-6 md:pl-8">
            {roles.length > 1 && <div className="absolute left-[7px] md:left-[9px] top-3 bottom-3 w-px bg-gradient-to-b from-amber-500/50 via-zinc-800 to-zinc-800/20" />}
            ...
            {displayRoles.map((role, rIndex) => (
              ...
              >
                <div className={`absolute -left-[23px] md:-left-[28px] top-1.5 w-3 h-3 rounded-full border-2 border-zinc-950 ring-4 ring-zinc-950/50 ${rIndex === 0 ? "bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]" : "bg-zinc-700"}`} />

                <div className="flex flex-col gap-4">
                  <div className="flex flex-wrap items-center gap-4">
                    <h4 className="text-base md:text-xl font-bold text-zinc-100 tracking-tight">{role.title}</h4>
                    <span className="chip bg-amber-500/5 border-amber-500/20 text-amber-400 px-3 py-1 text-[10px] md:text-xs">{role.period}</span>
                  </div>
                    ...
                    <ul className="space-y-3 md:space-y-4">
                      {role.points.map((point, pIndex) => (
                        <li key={pIndex} className="flex gap-4 text-sm md:text-[0.95rem] leading-relaxed text-zinc-400/80 group/point">
                          <span className="shrink-0 mt-2 w-1.5 h-1.5 rounded-full bg-amber-500/40 group-hover/point:bg-amber-500 transition-colors" />
            ...
            {hasMore && (
              <button
                ...
                className="min-h-11 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-amber-500 hover:text-amber-400 transition-all hover:gap-3"
              >
        ...
        <div className={`flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-12 md:mb-16 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="space-y-4">
            <div className="section-kicker bg-amber-500/10 border-amber-500/20">
              ...
              <span className="text-amber-300">Experience Ledger</span>
            </div>
