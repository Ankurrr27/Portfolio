...
const parseInline = (text) => {
  ...
  const rules = [
    ...
    },
    {
      name: 'underline',
      regex: /__(.*?)__/,
      render: (content) => <u className="underline decoration-orange-500/50">{parseInline(content)}</u>,
    },
    {
      name: 'link',
      regex: /\[(.*?)\]\((.*?)\)/,
      render: (content, url) => (
        <a 
          ...
          className="text-orange-500 hover:text-orange-400 underline decoration-orange-500/30 transition-colors inline"
        >
          {parseInline(content)}
        </a>
      ),
